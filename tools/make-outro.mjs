...
/* This story's own closing take, generated by tools/outro-shot.mjs.

   Preferred over anything shared. The era fallback below returns the era's *longest* clip, which
   is one deterministic answer for the whole era — eleven of twelve Gupta outros were the same
   file, byte for byte, before this existed. */
async function ownTake(slug) {
  const dir = path.join('episodes', slug, 'outro-clips');
  const files = (await readdir(dir).catch(() => [])).filter((f) => f.endsWith('.mp4'));
  if (!files.length) return null;
  let best = null; let n = 0;
  for (const f of files) {
    const g = f.match(/-r(\d+)\.mp4$/);
    if (g && Number(g[1]) >= n) { n = Number(g[1]); best = f; }
  }
  return best ? path.join(dir, best) : null;
}
...
const own = closer.take ? null : await ownTake(SLUG);
const src = closer.take
  ? await newest(closer.take[0], closer.take[1])
  : (own || await eraTake(ERA));
if (!src) {
  console.error(closer.take
    ? `missing take: ${closer.take.join('/')}`
    : `no closing take for ${SLUG} and no abstract clips in eras/${ERA}/clips —`
      + ` run tools/outro-shot.mjs --slug ${SLUG}`);
  process.exit(1);
}
/* Falling back to a shared beat is allowed and must never be silent: it is the difference
   between an ending that belongs to this story and the same fifteen seconds every viewer has
   already seen. */
if (!closer.take && !own) {
  console.warn(`  ! ${SLUG} has no closing take of its own — falling back to a shared ${ERA} beat,`);
  console.warn(`    which every other ${ERA} episode also closes on. Run tools/outro-shot.mjs --slug ${SLUG}`);
}
