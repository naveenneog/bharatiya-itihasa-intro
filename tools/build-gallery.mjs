...
/* Masters are written to dist/, not into the version folder, so they are attached
   here. v4 is listed as its own version because it is a separate deliverable file,
   but it shares v3's stills and clips rather than duplicating fourteen generations
   of them — the picture is identical, the score is what is new. */
const v3 = versions.find((v) => v.id === 'v3-empires-ink');
if (v3) {
  try { await stat('dist/v3-empires-ink.mp4'); v3.builds.push('../dist/v3-empires-ink.mp4'); } catch { /* not rendered */ }
}
try {
  await stat('dist/v4-empires-scored.mp4');
  versions.push({
    id: 'v4-empires-scored',
    dir: '.',
    play: 'versions/v3-empires-ink/build/index.html',
    meta: {
      name: 'v4 — the Empires, scored',
      pitch: 'The v3 picture with a synthesised score muxed in. No samples and no audio files: '
        + 'a tanpura-like drone, a tabla pulse and a plucked line, generated by the same Web Audio '
        + 'synth v1 uses and rendered offline through an OfflineAudioContext.',
      motion: 'The pulse enters on Indus, tightens through three gears — vilambit, madhya, drut — '
        + 'and stops dead on the wordmark, so the acceleration is heard as well as seen.',
    },
    beats: new Map(),
    clips: [],
